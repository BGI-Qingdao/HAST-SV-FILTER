...
do
  ...
  " >> process_log
  samtools depth -a -r ${ch_id}:${_start_upstream}-${_end_downstream} ${bamfile} >${prefix}.cov
  echo "
  java -jar ${src}/region_depth_avg.jar ${prefix}.cov ${ch_id} ${_start} ${_end} ${_start_upstream} ${_end_downstream} ${sv_id} ${threshold1} ${threshold2}
  " >> process_log
  java -jar ${src}/region_depth_avg.jar ${prefix}.cov ${ch_id} ${_start} ${_end} ${_start_upstream} ${_end_downstream} ${sv_id} ${threshold1} ${threshold2} >>${prefix}.del.filtered.bed
  count=$[count+1]
done <${prefix}.del.bed

echo "A total of ${count} SVs have been processed..."
